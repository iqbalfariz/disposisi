...
<input type="file" multiple="" class="form-control" name="img[]" id="file2">
		...
		<div>
		<?php 
			$sqldisposisi="select * from tbl_jabatan_login where id='1' and kode_jabatan!='admin'  and unit_kerja!='PA' order by id2 asc";
			$eksdisposisi=pg_query($sqldisposisi)or die($sqldisposisi);
			while($rd=pg_fetch_array($eksdisposisi)){
				if($rd[kode_jabatan]=="DU"){
					$kode="DU";
				}elseif($rd[kode_jabatan]=="DWB"){
					$kode="DWB";
				}elseif($rd[kode_jabatan]=="DJT"){
					$kode="DJT";
				}elseif($rd[kode_jabatan]=="DWT"){
					$kode="DWT";
				}elseif($rd[kode_jabatan]=="DPB"){
					$kode="DPB";
				}elseif($rd[kode_jabatan]=="DKU"){
					$kode="DKU";				
				}elseif($rd[kode_jabatan]=="SP"){
					$kode="SP";
				}
				//echo $kode;
				echo "<label class='' id='".$rd['kode_jabatan']."'>
					<div class='icheckbox_square-grey' style='position: relative;'><input type='checkbox' value='".$rd['kode_jabatan']."' class='input-themed' name='ke[]' id='general-themed-checkbox2' style='position: absolute; opacity: 0;'><ins class='iCheck-helper' style='position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; background: none repeat scroll 0% 0% rgb(255, 255, 255); border: 0px none; opacity: 0;'></ins></div>&nbsp;&nbsp;".$kode."
					</label>
					</div>
					<div>";
			}
			...
			$sqldisposisi="select * from tbl_jabatan_login where id<=7 and kode_jabatan!='admin' and kode_jabatan!='DU' and unit_kerja!='PA' order by id2 asc";
			$eksdisposisi=pg_query($sqldisposisi)or die($sqldisposisi);
			while($rd=pg_fetch_array($eksdisposisi)){
				if($rd[kode_jabatan]=="DU"){
					$kode="DU";
