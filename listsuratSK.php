...
<table cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-hover" id="example-table">
	<thead>
		<tr style="color:#black;background-color:#2980B9;">
			...
			<th style="color:#fff;background-color:#2980B9; ">Nomor Agendaa</th>
	...
	<tbody>
		<?php
			$sql="select a.*,b.*,c.pengirim as pengirimnya from tbl_sekertaris_lantai a left join tbl_sekertariat_disposisi b on a.nomor_agenda=b.nomor_agenda left join tbl_kode_pengirim c on b.pengirim=c.kode_pengirim where a.lantai='".substr($_SESSION[uname],3,1)."'";
			//echo $sql;
			$eks=pg_query($sql);
			if($eks){
			$no=1;
		...
		<tr class="odd gradeX">
			...
			<td><a href="<?php echo $app;?>?detail=bacapesanPA&id=<?php echo $rs['nomor_agenda'];?>&dari=<?php echo "Sekertariat";?>&ke=<?php echo $rs['gm'];?>"><?php echo $rs['nomor_agenda'];?></a></td>
			...
			<td class="hidden-xs hidden-sm"><a href="<?php echo $app;?>/?disdetail=disposisidetail&id=<?php echo $rs['gm'];?>&no=<?php echo $rs['nomor_agenda'];?>"><?php echo $rs['gm'];?></a></td>
			<td class="hidden-xs hidden-sm"><?php echo "sekertariat";?></td>
