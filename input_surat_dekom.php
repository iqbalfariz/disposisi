...
$date=date("Y-m-d H:i:s");
	if(isset($_POST['save'])){
		...
		$timesnya=date("H:i:s");
		$tglmasuksrt=$tglmasuksrt." ".$timesnya;
			...
			if($tglbatastindak>=$tglbatassurat){
				...
				if($ekscek < 0){
					...
					foreach ($_POST['ke'] as $key => $value) {
						$kenya[] = $ke[$key];
						$sqldis="insert into tbl_disposisi (id_disposisi,nomor_agenda,tgl_masuk_surat,dari,ke,cc,catatan,status,tgl_kirim,jns)
									values(nextval('tbl_disposisi_id_disposisi_seq'::regclass),'$_POST[kd]','$tglmasuksrt','Sekertariat','$ke[$key]','','','0','$date','1')";
						//echo $sqldis;
					...
					$sqlsimpan="INSERT into tbl_sekertariat_disposisi (id,nomor_agenda,sifat,jenis,kategori,tgl_terima,pengirim,tgl_surat,no_surat,perihal,user_add,ip_add,waktu_add,ke,batas_surat,bts_tindak_lanjut,pengirim_surat,jns)
								values(nextval('tbl_sekertariat_disposisi_id_seq'::regclass),'$_POST[kd]','$_POST[sifat]','$_POST[jenis]','$_POST[kat]','$tglmasuksrt',
									'$_POST[pengirim]',$tglsurat,'$_POST[no_surat]','$_POST[perihal]','$_SESSION[id]','$ipaddress','$datetime','$disposisi_ke','$tglbatassurat','$tglbatastindak','$_POST[pengirim_surat]','1')";
					//echo $sqlsimpan;
...
<div class="block block-themed">
    ...
    <span class="add-on">
      <i data-time-icon="icon-time" data-date-icon="icon-calendar">
		...
		<div>
			<?php 
			$sqldisposisi="select * from tbl_jabatan_login where  kode_jabatan like '%KOM%' and kode_jabatan !='SEKDEKOM' order by id2 asc";
		...
		<?php 
			$sqldisposisi="select * from tbl_jabatan_login where id IN(5010,5011,5012,5013,5014,5015,5016) order by id2 asc";
			$eksdisposisi=pg_query($sqldisposisi)or die($sqldisposisi);
			while($rd=pg_fetch_array($eksdisposisi)){
				echo "<label class='' id='".$rd['kode_jabatan']."'>
					<div class='icheckbox_square-grey' style='position: relative;'><input type='checkbox' value='".$rd['kode_jabatan']."' class='input-themed' name='ke[]' id='general-themed-checkbox2' style='position: absolute; opacity: 0;'><ins class='iCheck-helper' style='position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; background: none repeat scroll 0% 0% rgb(255, 255, 255); border: 0px none; opacity: 0;'></ins></div>&nbsp;&nbsp;".$rd['kode_jabatan']."
